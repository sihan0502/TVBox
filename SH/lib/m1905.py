...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def searchContent(self,key,quick):
        ...
        rsp = self.fetch(url, headers=headers)
        html = self.html(rsp.text)
        aList = html.xpath("//div[contains(@class,'subject-movie-box')]")
        videos = []
        for a in aList:
            aids = a.xpath(".//a[contains(@href,'vod')]/@href") #https://www.1905.com/vod/play/85646.shtml
            if aids and aids[0]:
              aid = self.regStr(reg=r'play/(.*?).sh', src=aids[0]) # 85646
              img = a.xpath('./div/div/a/img/@src')[0]
              title = a.xpath('./div/div/a/img/@alt')[0]
              videos.append({
                "vod_id": aid,
                "vod_name": title,
                "vod_pic": img,
